...
const createBooking = async (req, res) => {
    try {
        ...
        // Validate driver ID if driver is required
        if (driverRequired && driverId && !mongoose.Types.ObjectId.isValid(driverId)) {
            return res.status(400).json({
                success: false,
        ...
        if (driverRequired && driverId) {
            const conflictingDriverBookings = await Booking.find({
            ...
            if (conflictingDriverBookings.length > 0) {
                return res.status(400).json({
                    success: false,
                    message: 'Selected driver is not available for the requested dates'
                });
            }
        }
        ...
        const booking = new Booking({
            ...
            driver: {
                ...
                driverId: (driverRequired && driverId) ? driverId : null,
                driverFee: driverFee
            },
...
const getClientPaymentHistory = async (req, res) => {
    try {
        ...
        console.log('Getting payment history for client:', clientId);
        ...
        // Build query for bookings with payments
        const query = {
            clientId,
        ...
        const bookings = await Booking.find(query)
            ...
            .populate('vehicleOwnerId', 'firstName lastName')
            .sort({ 'payment.paymentDate': -1, createdAt: -1 })
            .limit(parseInt(limit));
        ...
        const paymentHistory = bookings.map(booking => ({
            ...
            vehicle: `${booking.vehicleId?.make || ''} ${booking.vehicleId?.model || ''}`.trim(),
            vehicleYear: booking.vehicleId?.year || '',
            licensePlate: booking.vehicleId?.licensePlate || '',
            destination: `${booking.dropoffLocation?.city || ''}, ${booking.dropoffLocation?.address || ''}`.trim(),
            pickupLocation: `${booking.pickupLocation?.city || ''}, ${booking.pickupLocation?.address || ''}`.trim(),
            amount: booking.payment?.paidAmount || booking.pricing?.totalAmount || 0,
        ...
        console.log(`Found ${paymentHistory.length} payment records for client`);
