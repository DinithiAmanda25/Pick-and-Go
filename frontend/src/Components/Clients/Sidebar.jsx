...
function ClientSidebar() {
  ...
  const menuItems = [
    ...
    {
      name: 'My Cards',
      path: '/client-dashboard?tab=cards',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
        </svg>
  ...
  return (
    <div className="bg-blue-900 text-white w-64 h-screen p-4 flex flex-col overflow-y-auto">
      <div className="mb-8 flex flex-col items-center">
        ...
        <p className="text-blue-200 text-base mt-2">Client Portal</p>
      </div>
      ...
      <nav className="space-y-2 flex-1 overflow-y-auto">
        {menuItems.map((item) => {
          ...
          } else if (item.path === '/client-dashboard?tab=cards') {
            isActive = currentTab === 'cards'
      ...
      <div className="mt-4 pt-4 border-t border-blue-700 flex-shrink-0">
        <button
          onClick={handleLogout}
          className="flex items-center space-x-3 px-4 py-3 rounded-lg text-blue-200 hover:bg-red-600 hover:text-white transition-colors duration-200 w-full text-left"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
