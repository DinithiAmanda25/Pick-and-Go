...
class BookingService {
  ...
  // Get all bookings for admin (system-wide)
  async getAllBookings(status = null, page = 1, limit = 100) {
    try {
      ...
      const url = `/bookings/admin/all?${params.toString()}`;
      const response = await api.get(url);
      return response.data;
    } catch (error) {
  ...
  async getDriverBookings(driverId, status = null, startDate = null, endDate = null) {
    try {
      const params = new URLSearchParams();
      ...
      const url = `/bookings/admin/driver-schedule?${params.toString()}`;
      const response = await api.get(url);
      return response.data;
    } catch (error) {
      console.error('Error fetching driver bookings:', error);
      throw new Error(error.response?.data?.message || 'Failed to fetch driver bookings');
    }
  }
